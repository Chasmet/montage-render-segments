...
function splitTextIntoParts(raw){const text=txt(raw);if(!text)return[];return text.split(/(?<=[.!?…])\s+/).map(x=>txt(x)).filter(Boolean).slice(0,MAX_LINES)}
function approxSegments(text){const parts=splitTextIntoParts(text);let t=0;return parts.map((p,i)=>{const duration=Math.max(1.6,Math.min(5,p.length/18));const item={start:t,end:t+duration,character:'Voix '+String.fromCharCode(65+i),text:p,voice:voiceGuess(p)};t+=duration+0.25;return item})}
...
async function mp4dub(input,audio,out){await ff(['-y','-hide_banner','-loglevel','warning','-i',input,'-i',audio,'-map','0:v:0','-threads','1','-filter_complex',"[0:a]asplit=2[bgduck][bgmix];[1:a]volume=1.35,asplit=2[dubduck][dubmix];[bgduck][dubduck]sidechaincompress=threshold=0.02:ratio=20:attack=8:release=220[duck];[duck][dubmix]amix=inputs=2:duration=first:normalize=0[aout]",'-map','[aout]','-filter:v',"scale='min(720,iw)':-2,fps=25,format=yuv420p",'-c:v','libx264','-preset','ultrafast','-crf','30','-c:a','aac','-b:a','128k','-ar','44100','-ac','2','-movflags','+faststart',out],'mp4 double ambiance')}
app.get('/api/health',(req,res)=>res.json({ok:true,service:'openai-dubbing-mp4-v2',openaiReady:Boolean(openai),ttsModel:TTS_MODEL,transcribeModel:TRANSCRIBE_MODEL,audioMode:'normal-background-ai-cut',maxUploadMb:MAX_UPLOAD_MB,maxDialogueLines:MAX_LINES,busy}));
app.get('/',(req,res)=>res.json({ok:true,message:'Backend V2 actif',openaiReady:Boolean(openai),audioMode:'normal-background-ai-cut'}));
app.post('/api/analyze-video',upload.single('video'),async(req,res)=>{if(!openai)return res.status(500).json({success:false,error:'OPENAI_API_KEY manquante dans Render.'});const job=path.join(JOBS,'analyse-'+uuid());let input=req.file?.path;try{if(!input)return res.status(400).json({success:false,error:'Aucune vidéo reçue.'});await mkdir(job);const audio=path.join(job,'audio.mp3');await extractAudio(input,audio);let tr;try{tr=await openai.audio.transcriptions.create({file:fs.createReadStream(audio),model:TRANSCRIBE_MODEL,response_format:'verbose_json',timestamp_granularities:['segment']})}catch(firstError){tr=await openai.audio.transcriptions.create({file:fs.createReadStream(audio),model:TRANSCRIBE_MODEL,response_format:'json'})}const raw=Array.isArray(tr.segments)?tr.segments:[];let segments=raw.slice(0,MAX_LINES).map((s,i)=>{const text=txt(s.text);const character='Voix '+String.fromCharCode(65+i);return{start:Number(s.start||0),end:Number(s.end||0),character,text,voice:voiceGuess(text)}}).filter(s=>s.text);if(!segments.length)segments=approxSegments(tr.text||'');const script=segments.map(s=>'['+stamp(s.start)+'] '+s.character+' : '+s.text).join('\n');const voiceMap={};segments.forEach(s=>voiceMap[s.character]=s.voice);await rm(input);input=null;setTimeout(()=>rm(job),CLEANUP_MS);res.json({success:true,message:'Analyse terminée. Corrige si besoin puis génère.',segments,script,voiceMap,estimatedTiming:!raw.length})}catch(e){await rm(input);await rm(job);res.status(500).json({success:false,error:e.message||'Erreur analyse vidéo.'})}});
app.post('/api/process-video',upload.single('video'),async(req,res)=>{if(busy){await rm(req.file?.path);return res.status(429).json({success:false,error:'Le serveur traite déjà une vidéo.'})}busy=true;const jobId=uuid();const job=path.join(JOBS,jobId);let input=req.file?.path;try{if(!input)return res.status(400).json({success:false,error:'Aucune vidéo reçue.'});await mkdir(job);const out=path.join(job,'final-'+safe(req.file.originalname));const inStat=await fsp.stat(input);const dub=await dubbing(req.body.script,map(req.body.voiceMap),job);if(dub.audioPath)await mp4dub(input,dub.audioPath,out);else await mp4(input,out);await rm(input);input=null;const outStat=await fsp.stat(out);setTimeout(()=>rm(job),CLEANUP_MS);res.json({success:true,message:dub.audioPath?'MP4 doublé : Grok normal, coupé seulement pendant OpenAI.':'MP4 généré sans doublage.',jobId,renderMode:dub.audioPath?'openai-dubbing-v2-normal-background-ai-cut':'light',dubbing:{enabled:Boolean(dub.audioPath),lines:dub.lines,characters:dub.characters,timed:Boolean(dub.timed),backgroundKept:Boolean(dub.audioPath),audioDownloadUrl:dub.audioPath?'/downloads/'+jobId+'/doublage.mp3':null},input:{sizeMb:mb(inStat.size)},output:{sizeMb:mb(outStat.size)},downloadUrl:'/downloads/'+jobId+'/'+path.basename(out)})}catch(e){await rm(input);await rm(job);res.status(500).json({success:false,error:e.message||'Erreur traitement vidéo.'})}finally{busy=false}});
...
async function boot(){await mkdir(TMP);await mkdir(UPLOADS);await mkdir(JOBS);app.listen(PORT,()=>console.log('Backend V2 actif sur port '+PORT+' audioMode=normal-background-ai-cut'))}
boot();
