...
async function mp4dub(input,audio,out){await ff(['-y','-hide_banner','-loglevel','warning','-i',input,'-i',audio,'-map','0:v:0','-threads','1','-filter_complex',"[0:a]volume=0.55[bg];[1:a]volume=1.35,asplit=2[dubduck][dubmix];[bg][dubduck]sidechaincompress=threshold=0.03:ratio=10:attack=20:release=350[duck];[duck][dubmix]amix=inputs=2:duration=first:normalize=0[aout]",'-map','[aout]','-filter:v',"scale='min(720,iw)':-2,fps=25,format=yuv420p",'-c:v','libx264','-preset','ultrafast','-crf','30','-c:a','aac','-b:a','128k','-ar','44100','-ac','2','-movflags','+faststart',out],'mp4 double ambiance')}
app.get('/api/health',(req,res)=>res.json({ok:true,service:'openai-dubbing-mp4-v2',openaiReady:Boolean(openai),ttsModel:TTS_MODEL,transcribeModel:TRANSCRIBE_MODEL,audioMode:'background-ducking-v2',maxUploadMb:MAX_UPLOAD_MB,maxDialogueLines:MAX_LINES,busy}));
app.get('/',(req,res)=>res.json({ok:true,message:'Backend V2 actif',openaiReady:Boolean(openai),audioMode:'background-ducking-v2'}));
...
async function boot(){await mkdir(TMP);await mkdir(UPLOADS);await mkdir(JOBS);app.listen(PORT,()=>console.log('Backend V2 actif sur port '+PORT+' audioMode=background-ducking-v2'))}
boot();
